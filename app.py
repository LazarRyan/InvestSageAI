...
def init_db():
    conn = sqlite3.connect('investsage.db')
    c = conn.cursor()
    c.execute('''
...
# Modified portfolio file path to include user ID
def get_portfolio_path(user_id=None):
    if user_id:
        # Create user directory if it doesn't exist
        user_dir = os.path.join(DATA_DIR, "users", user_id)
        os.makedirs(user_dir, exist_ok=True)
        return os.path.join(user_dir, "portfolio.csv")
    else:
        # Guest portfolio is temporary
        return os.path.join(DATA_DIR, "guest_portfolio.csv")

# Authentication UI
def show_login_page():
    st.title("Welcome to InvestSageAI")
    ...
    with tab3:
        ...
        st.info("In guest mode, your portfolio will not be saved between sessions.")
...
def create_empty_portfolio_file(user_id=None):
    portfolio_file = get_portfolio_path(user_id)
    try:
...
def load_portfolio(user_id=None):
    ...
    try:
        ...
        create_empty_portfolio_file(user_id)
...
def save_portfolio(portfolio_df, user_id=None):
    try:
        ...
        if user_id:
            backup_path = os.path.join(DATA_DIR, "users", user_id, "portfolio_backup.csv")
        else:
...
def add_to_portfolio(ticker_data, ticker, shares, purchase_price, user_id=None):
    try:
        # Get portfolio file path
        file_path = get_portfolio_path(user_id)
        ...
        try:
            ...
            try:
                ...
                success, direct_path = direct_add_to_portfolio(ticker, purchase_price, shares, user_id)
                if success:
                    try:
...
def direct_add_to_portfolio(ticker, price, shares, user_id=None):
    ...
    try:
        ...
        if user_id:
            user_dir = os.path.join(DATA_DIR, "users", user_id)
            os.makedirs(user_dir, exist_ok=True)
            file_path = os.path.join(user_dir, "portfolio_direct.csv")
        else:
...
def remove_from_portfolio(ticker, user_id=None):
    try:
        ...
        if save_portfolio(st.session_state.portfolio, user_id):
...
def main():
    ...
    with st.sidebar:
        if st.session_state.is_guest:
            st.info(f"You are using InvestSageAI as a Guest. Your data won't be saved.")
        else:
    ...
    if 'portfolio' not in st.session_state:
        st.session_state.portfolio = load_portfolio(st.session_state.user_id)
    
    # Sidebar
    st.sidebar.title("Navigation")
    page = st.sidebar.radio("Go to", ["Portfolio Dashboard", "Asset Explorer & Analysis", "Portfolio Grading"])
    ...
    if page == "Portfolio Dashboard":
            ...
            with col1:
                if st.button("Refresh Prices"):
                    with st.spinner("Updating prices..."):
                        ...
                        save_portfolio(portfolio_df, st.session_state.user_id)
                        st.session_state.portfolio = portfolio_df
                        st.success("Prices updated successfully!")
            
            with col2:
                if not portfolio_df.empty:
                    asset_to_remove = st.selectbox("Remove Asset", [""] + portfolio_df["Ticker"].tolist())
                    if asset_to_remove and st.button("Remove"):
                        if remove_from_portfolio(asset_to_remove, st.session_state.user_id):
                            st.rerun()
            
            if portfolio_df.empty:
                st.info("Your portfolio is empty. Go to Asset Explorer to add assets.")
            else:
                ...
                col4.metric("Total Return", f"{total_profit_loss_pct:.2f}%")
                ...
                currency_cols = ['Current Price', 'Purchase Price', 'Total Value', 'Profit/Loss']
                for col in currency_cols:
                    display_df[col] = display_df[col].apply(lambda x: f"${x:,.2f}")
                
                # Format percentage columns
                pct_cols = ['Profit/Loss (%)', 'YoY Growth (%)', 'Dividend Yield (%)']
                for col in pct_cols:
                    display_df[col] = display_df[col].apply(lambda x: f"{x:.2f}%")
                ...
                viz_tab1, viz_tab2, viz_tab3 = st.tabs(["Allocation", "Performance", "Risk vs. Return"])
                ...
                with viz_tab2:
                    ...
                    fig = px.bar(
                        ...
                        x='Ticker',
                        y='Profit/Loss (%)',
                        title='Asset Performance (% Return)',
                        color='Profit/Loss (%)',
                        color_continuous_scale=px.colors.diverging.RdYlGn,
                        labels={'Profit/Loss (%)': 'Return (%)'},
                        hover_data=['Name', 'Purchase Price', 'Current Price']
                    )
                    ...
                    fig = px.bar(
                        ...
                        x='Ticker',
                        y='YoY Growth (%)',
                        title='Asset Year-over-Year Growth',
                        color='YoY Growth (%)',
                        color_continuous_scale=px.colors.diverging.RdYlGn,
                        ...
                        if not ticker or shares <= 0 or purchase_price <= 0:
                            ...
                            if ticker_data:
                                ...
                                if not portfolio_df.empty and ticker in portfolio_df["Ticker"].values:
                                    ...
                                    # Add to portfolio
                                    if add_to_portfolio(ticker_data, ticker, shares, purchase_price, st.session_state.user_id):
        ...
        with analyze_tab:
            ...
            # Initialize ticker_to_analyze variable
