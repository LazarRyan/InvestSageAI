...
# Using pandas for CSV operations (pd.read_csv, pd.DataFrame.to_csv)
...
# Simplified function to load portfolio using pandas
def load_portfolio():
    """Load portfolio data from CSV file using pandas"""
    try:
        if os.path.exists(PORTFOLIO_FILE) and os.path.getsize(PORTFOLIO_FILE) > 0:
            portfolio_df = pd.read_csv(PORTFOLIO_FILE)
            logger.info(f"Successfully loaded portfolio with {len(portfolio_df)} rows")
            return portfolio_df
        else:
            logger.info("Creating new empty portfolio")
            return pd.DataFrame(columns=DEFAULT_PORTFOLIO_COLUMNS)
    except Exception as e:
        logger.error(f"Error loading portfolio: {e}")
        return pd.DataFrame(columns=DEFAULT_PORTFOLIO_COLUMNS)

# Simplified function to save portfolio using pandas
def save_portfolio(portfolio_df):
    """Save portfolio data to CSV file using pandas"""
    try:
        # Save DataFrame to CSV
        portfolio_df.to_csv(PORTFOLIO_FILE, index=False)
        logger.info(f"Portfolio saved to: {os.path.abspath(PORTFOLIO_FILE)}")
        return True
    except Exception as e:
        ...
        try:
            ...
            logger.info(f"Portfolio saved to backup location: {os.path.abspath(backup_path)}")
            return True
        except Exception as backup_error:
            logger.error(f"Error saving to backup location: {backup_error}")
            return False

# Simplified function to add asset to portfolio
def add_to_portfolio(ticker_data, ticker, shares, purchase_price):
    ...
    try:
        ...
        if save_portfolio(portfolio_df):
            ...
            logger.info(f"Added {ticker} to portfolio, now has {len(portfolio_df)} rows")
            return True
        else:
            logger.error(f"Failed to save portfolio after adding {ticker}")
            return False
...
# Simplified function to remove asset from portfolio
def remove_from_portfolio(ticker):
    ...
    try:
        ...
        st.session_state.remove_success = {"status": "error", "message": f"Error removing {ticker}: {e}"}
        return False

# Function to update portfolio prices
def update_portfolio_prices():
    """Update current prices and calculations in the portfolio"""
    try:
        portfolio_df = load_portfolio()
        ...
        for index, row in portfolio_df.iterrows():
            ...
            if stock_data and "financials" in stock_data:
                # Update values
                current_price = float(stock_data["financials"].get("Current Price", 0))
                total_value = shares * current_price
                profit_loss = shares * (current_price - purchase_price)
                profit_loss_pct = ((current_price - purchase_price) / purchase_price * 100) if purchase_price > 0 else 0
                
                # Update the row
                portfolio_df.at[index, 'Current Price'] = current_price
                portfolio_df.at[index, 'Total Value'] = total_value
                portfolio_df.at[index, 'Profit/Loss'] = profit_loss
                portfolio_df.at[index, 'Profit/Loss (%)'] = profit_loss_pct
                portfolio_df.at[index, 'YoY Growth (%)'] = float(stock_data["financials"].get("YoY Growth", 0))
        ...
        if save_portfolio(portfolio_df):
            ...
            logger.error("Failed to save portfolio after updating prices")
            return False
    except Exception as e:
